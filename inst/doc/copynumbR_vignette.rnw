...
\subsection{Visualization of the complete data}
...
gold-standard for identifying recurrent copy number alterations in a cohort. We
provide a few helper functions for parsing GISTIC output. We load the significant GISTIC peaks, the genomic loci which are found in significantly more patients than expected by chance, and their copy numbers:
...
@
GISTIC further identifies potential driver genes of recurrent focal alterations. We can get these genes with the 
...
We can get a quick overview of all focal amplifcation with the
...
@
GISTIC is also helpful in idenfying broad events, which span over a specified percentage of a whole chromosome arm. A nice way of visualizing these broad events is with our armplot (Fig.~\ref{fig:armplot}):
...
\section{Matched Expression Data}
...
copynumbR.boxplot(eset.genes[,isc], PMID17099711.GPL91_eset[,isc],
    probeset=c("MYC", "ADCY8"))+geom_jitter()
@
